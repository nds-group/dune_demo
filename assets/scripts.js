  ...
  window.intervalId = setInterval(() => {
    ...
    if (perfData && chartSeriesDataFunction && performanceUpdateFunction) {
      ...
      chartLine.updateOptions({
        xaxis: { min: now.getTime() - (15 * 60 * 1000), max: now.getTime() }
      }, false, false);
...
async function initializeApp() {
  ...
  // TODO: Update this part and get data from .json file (this is hardcoded)
  window.dunePerfData = {
  ...
  setInterval(function () {
    ...
    if (window.state_demo === 0) { // Only update if idle
      ...
      chartLine.updateOptions({
        xaxis: {
          min: now.getTime() - (10 * 60 * 1000), // 10 min ago
          max: now.getTime()
        }
      }, false, false);
